...
namespace AutoCatalogLib.Business.CatalogSearch
{
    class ExactSearch : SearchBase
    {
        ...
        {
            return SqlFormatter.Conjunction(tokens.Select(t => SqlFormatter.Disjunction(
                SqlFormatter.FormatStartsWith("SearchString", t + " "),
                SqlFormatter.FormatContains("SearchString", " " + t + " ")))
            );
        }
        ...
        protected override IQueryable<PartItem> FilterItemsByToken(IQueryable<PartItem> items, string token)
        {
            return
                from item in items
                where 
                    item.SearchString.StartsWith(token + " ") ||
                    item.SearchString.Contains(" " + token + " ")
                select item;
        }
    }
}